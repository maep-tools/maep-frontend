...
export default {
  ...
  methods: {
    changePlant() {
      ...
      api.getSpeedIndicesById(this.windSelected).then((r) => {
    ...
    handleFuelUpdated(v) {
      ...
      if (_.isNil(v.fromRowData.wind_config_id)) {
        ...
        // manda crear
        api.createSpeedIndices(v.fromRowData).then((c) => {
          if (c.status === 200) {
        ...
        // manda modificar
        api.updateSpeedIndices(v.fromRowData.id, v.fromRowData).then((c) => {
          console.log(c)
        })
