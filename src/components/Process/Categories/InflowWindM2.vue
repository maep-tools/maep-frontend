...
export default {
  ...
  mounted() {
    ...
    if (this.data.windM2Config.length < 1 && !this.canEdit) {
    ...
    this.windConfig = this.data.windM2Config
  ...
  methods: {
    changePlant() {
      ...
      api.getInflowWindByIdM2(this.windSelected).then((r) => {
        _.each(speedIndices, (v) => {
          _.each(r.data, (vv) => {
            if (vv.horizont_id === v.id_horizont && vv.wind_config_id === this.windSelected && v.scenario === vv.scenario) {
              v.value = vv.value
    ...
    handleFuelUpdated(v) {
      ...
      v.fromRowData.horizont = v.fromRowData.id_horizont      
      ...
      if (_.isNil(v.fromRowData.wind_config_id)) {
        ...
        // manda crear
        api.createInflowWindM2(v.fromRowData).then((c) => {
          if (c.status === 200) {
        ...
        // manda modificar
        api.updateInflowWindM2(v.fromRowData.id, v.fromRowData).then((c) => {
          console.log(c)
        })
