...
<div>
  <el-dialog title="Nuevo"
    ...
    <div class="row">
      <div class="col-md-12">
        <label class="control-label">Nombre del escenario</label>
        <div class="form-group required">
          <input v-model="nameScenario"
                 type="text"
                 name="name"
                 class="form-control"
                 placeholder="Ingrese el nombre del escenario">
        </div>
      </div>
    </div>
    <span slot="footer"
    ...
    <el-button  @click="addRow()" :disabled="!nameScenario" type="primary">Continuar</el-button>
  </span>
  </el-dialog>
  <!-- MENU DE COMPONENTE -->
  <div class="row">
    <div class="col-md-12"
      ...
      <button @click="deleteRow()"
              v-if="!showUpload && rows.length > 0"
              style="margin-left:10px;"
              class="btn btn-sm btn-danger float-right"><i class="icon trash"></i>Reducir escenarios (-1)</button>
      <!--
  ...
  <div v-if="this.rows.length < 1">
    <center>
      <h2>No existen escenarios</h2>
      <img style="margin-bottom:10px;"
           width="100"
           src="/static/scenario.png">
      <hr>
...
export default {
  ...
  methods: {
    openModal() {
      ...
      this.nameScenario = 'Escenario ' + parseInt(this.rows.length + 1, 10)
    },
    ...
    */
    deleteRow() {
      ...
      api.deleteScenario(row.id).then((r) => {
    ...
    */
    handleGridRowsUpdated(data) {
      for (let i = data.fromRow; i <= data.toRow; i++) {
        ...
        api.updateScenario(updatedRow.id, updatedRow).then((r) => {
          if (r.status === 200) {
            ...
            this.$forceUpdate()
            this.$message({
              message: 'Error al modificar, el area ya existe.',
              type: 'error'
            })
