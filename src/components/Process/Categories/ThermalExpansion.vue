...
<div element-loading-text="Tienes que esperar un poco..."
  ...
  <div v-if="!showUpload && visibleTable && !loading">
    ...
    <div class="row">
      <div class="col-md-12" v-if="selectedRow">
          <el-alert
  ...
  <div class="row"
    ...
    <div v-for="column in allColumns"
      ...
      <div class="form-group required"
        ...
        <label class="control-label">{{column.name}}</label>
        <el-date-picker v-model="selectedRow[column.key]"
                        type="month"
  ...
  <el-dialog title="Nueva expansión"
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="form-group required">
          ...
          <el-select style="width:100%"
                     v-model="selectedThermalConfig">
            <el-option v-for="item in data.thermalConfig"
                       :key="item.id"
                       :label="item.name"
                       :value="item.id">
            </el-option>
    ...
    <el-button  @click="addRow()" :disabled="!selectedThermalConfig && selectedThermalConfigMonth" type="primary">Continuar</el-button>
  </span>
  </el-dialog>
</div>
...
export default {
  ...
  mounted() {
    ...
    if (this.data.thermalConfig.length < 1) {
      this.$alert('No puedes acceder a esta entidad porque no hay plantas termicas creadas. Vas a ser redirigido al modulo de plantas termicas.', 'Advertencia', {
        confirmButtonText: 'OK',
        cancelButtonText: 'Cancelar',
        type: 'error'
      }).then(() => {
        this.$parent.goToCategory(4)
      })
    }

    if (this.data.horizonts.length < 1) {
      this.$alert('No puedes acceder a esta entidad porque no hay horizontes creados. Vas a ser redirigido al modulo de demanda.', 'Advertencia', {
        confirmButtonText: 'OK',
        cancelButtonText: 'Cancelar',
        type: 'error'
      }).then(() => {
  ...
  methods: {
    ...
    onPropsChanged(newVal) {
      this.rows = _.clone(this.data.thermalConfigExpansion, true)
      if (newVal) {
        this.rows = newVal.thermalConfigExpansion
      }
      ...
      _.each(this.rows, (v, k) => {
        v.thermalName = v.thermal_config.name
    ...
    update() {
      api.updateThermalConfigExpansion(this.selectedRow.id, this.selectedRow).then(() => {
        this.$message({
    ...
    */
    addRow() {
      ...
      api.createThermalConfigExpansion({
        thermal_config_id: this.selectedThermalConfig,
    ...
    */
    deleteRow(row) {
      api.deleteThermalConfigExpansion(row.id).then(() => {
  ...
  data() {
    return {
      ...
      selectedThermalConfigMonth: null,
      selectedThermalConfig: null,
      loading: true,
      visibleTable: true,
      ...
      allColumns: [ // nombre de las columnas 
        {
          key: 'thermalName',
          originalForeign: 'thermal_config_id',
          name: 'Nombre',
          type: 'select',
          entity: 'thermalConfig',
          resizable: true,
          display: true,
          displayInForm: false
        },
